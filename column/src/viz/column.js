...
//
vizuly.viz.column = function (parent) {
    ...
    function measure() {
        ...
        scope.yScale.domain([0, d3.max(scope.data, function (data) {
            return d3.max(data, function (d) { return scope.y(d) + ((scope.layout == vizuly.viz.layout.STACKED) ? d.y0 : 0); })
        })]);
        ...
        if (scope.enableZoom) initializeZoom();
    ...
    function onZoom() {
        ...
        vizuly.core.util.setRange(scope.xScale,0,Math.round(size.width*scope.zoom.scale()));
        ...
        barGroup.attr("transform",function (d,i) { return "translate(" + (x + scope.xScale(scope.x(d))) + ",0)"})
          .selectAll(".vz-bar")
          .attr("x",function (d,i) { return (scope.layout == vizuly.viz.layout.STACKED) ? 0 : i * (barWidth * scope.zoom.scale() + (barWidth * scope.zoom.scale() * scope.padding)); } )
          .attr("width",function (d,i) { return (scope.layout == vizuly.viz.layout.STACKED) ? groupWidth * scope.zoom.scale() : barWidth * scope.zoom.scale()} );
